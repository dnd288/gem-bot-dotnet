using gem_bot_dotnet_sdk.Constants;
using GemBotSdk.Enum;
using GemBotSdk.Models;
using Sfs2X;
using Sfs2X.Core;
...
namespace gem_bot_dotnet_sdk.Abstracts;
...
public abstract class BotBase
{
    private SmartFox sfs;
        ...
        private void FindGame()
        {
            ...
            sfs.Send(new ExtensionRequest(Command.LOBBY_FIND_GAME, data));
        }
        ...
        private void OnExtensionResponse(BaseEvent evt)
        {
            ...
            switch (cmd)
            {
                ...
                case Command.START_TURN:
                    StartTurn(evtParam);
                    break;
                case Command.ON_SWAP_GEM:
                    SwapGem(evtParam);
                    break;
                case Command.ON_PLAYER_USE_SKILL:
                    HandleGems(evtParam);
                    break;
                case Command.PLAYER_JOINED_GAME:
                    this.sfs.Send(new ExtensionRequest(Command.I_AM_READY, new SFSObject(), room));
                    break;
                case "SEND_ALERT":
        ...
        public void SendFinishTurn(bool isFirstTurn)
        {
            ...
            log("sendExtensionRequest()|room:" + room.Name + "|extCmd:" + Command.FINISH_TURN + " first turn " + isFirstTurn);
            sendExtensionRequest(Command.FINISH_TURN, data);
        }
        ...
        public void SendCastSkill(Hero heroCastSkill)
        {
            ...
            log("sendExtensionRequest()|room:" + room.Name + "|extCmd:" + Command.USE_SKILL + "|Hero cast skill: " + heroCastSkill.Name);
            sendExtensionRequest(Command.USE_SKILL, data);
        }
        ...
        public void SendSwapGem()
        {
            ...
            log("sendExtensionRequest()|room:" + room.Name + "|extCmd:" + Command.SWAP_GEM + "|index1: " + indexSwap.Param1 + " index2: " + indexSwap.Param2);
            sendExtensionRequest(Command.SWAP_GEM, data);
        }
